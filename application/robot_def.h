...
// 云台参数
#define YAW_CHASSIS_ALIGN_ECD 5017 // 云台和底盘对齐指向相同方向时的电机编码器值,若对云台有机械改动需要修改
#define YAW_ECD_GREATER_THAN_4096 1 // ALIGN_ECD值是否大于4096,是为1,否为0;用于计算云台偏转角度
#define PITCH_HORIZON_ECD 2218      // 云台处于水平位置时编码器值,若对云台有机械改动需要修改
#define PITCH_MAX_ANGLE 40//40           // 云台竖直方向最大角度 (注意反馈如果是陀螺仪，则填写陀螺仪的角度)
#define PITCH_MIN_ANGLE -28//-27           // 云台竖直方向最小角度 (注意反馈如果是陀螺仪，则填写陀螺仪的角度)
// 发射参数
...
// 机器人底盘修改的参数,单位为mm(毫米)
#define WHEEL_BASE 335              // 纵向轴距(前进后退方向)
#define TRACK_WIDTH 360             // 横向轮距(左右平移方向)
...
typedef struct 
{   
    float Vision_yaw_tar; //目标角度
    float Vision_pitch_tar;

    float Vision_yaw; // 当前绝对坐标
    float Vision_pitch;

    float Vision_set_yaw; // 自瞄模式下给电机发送的值
    float Vision_set_pitch;   
